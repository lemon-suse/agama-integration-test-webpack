...
export class StorageSettingsPage {
  ...
  private readonly changeEncryptionLink = () => this.page.locator("::-p-aria(Change[role='link'])");

  //public readonly encryptionIsEnabledText = () =>
  // this.page.locator("::-p-aria(Encryption is enabled[role='tabpanel'])");
  public readonly encryptionIsEnabledText = () =>
    this.page.locator('role/tabpanel[name="Encryption is enabled"]');

  public readonly encryptionIsDisabledText = () =>
    this.page.locator('role/tabpanel[name="Encryption is disbled"]');
  ...
  private readonly resetToDefaultsMenuitem = () =>
    this.page.locator("::-p-text(Reset to defaults[role='menuitem'])");
  ...
  async resetToDefault(timeout: number = 30 * 1000) {
    await this.resetToDefaultsMenuitem().setTimeout(timeout).click();
  }
